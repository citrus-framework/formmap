...
trait Datetime
{
    ...
    public static function varTypeDate(Element $element, $var): void
    {
        ...
        // ここまでに返却されてない場合はエラー
        $format = '「%s」には年月日を「yyyy-mm-dd」「yyyy/mm/dd」「yyyymmdd」のいずれかの形式で入力してください。';
        throw new FormmapException(sprintf($format, $element->name));
    }
    ...
    public static function varTypeTime(Element $element, $var): void
    {
        FormmapException::exceptionIf(
            (0 === preg_match('/^[0-9]{2}:[0-5][0-9]:?([0-5][0-9])+?/', $var)),
